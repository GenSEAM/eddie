...
(df test-simple-mode-routing [] -> Bool
  ...
  (let [(manifest (pol/make-manifest "/tmp" (list) "/tmp" false))
    ...
    (assert-case "c-mode-pos-001" (string-starts-with? out "SIMPLE-MODE"))
    (refute-case "c-mode-pos-001" (string-starts-with? out "MAINTAINER-MODE"))
    true))
...
(df test-maintainer-mode-routing [] -> Bool
  ...
  (let [(manifest (pol/make-manifest "/tmp" (list) "/tmp" false))
    ...
    (assert-case "c-mode-pos-002" (string-starts-with? out "MAINTAINER-MODE"))
    (refute-case "c-mode-pos-002" (string-starts-with? out "SIMPLE-MODE"))
    true))
...
(df test-cli-simple-flag-selection [] -> Bool
  ...
  (let [(opt-simple (cli/make-simple-cli-options (pol/level-auto) "gemma-4-31b-it" "quick-cmd" "" false true))
    ...
    (assert (.-is-simple opt-simple))
    (refute (.-is-simple opt-default))
    true))
