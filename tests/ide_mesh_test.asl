...
(df test-ide-task-formatting [] -> Bool
  (let [(t-claude (im/make-ide-task "T101" (im/ide-claude-code) "dev" "implement auth module" "/tmp/wt1" 1773900000000))
        ...
        (mcp-anti (im/format-antigravity-mcp-payload t-anti))
    ...
    (assert (string-contains? mcp-anti ":agy-mcp-request") "antigravity frame contains agy mcp header")
    (assert (string-contains? mcp-anti "orchestrate test suites") "antigravity frame includes prompt")
